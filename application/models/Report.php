...
class Report extends CI_Model {
	...
	public function get_content($order_id) {
		...
		$this -> pdo -> join('gene_categories','reports.id = gene_categories.id');
		$this -> pdo -> join('report_categories','reports.id = report_categories.id');
		
		$result['total'] = $this -> pdo -> count_all_results('reports');
	
		$this -> pdo -> select('gene_categories.title as gene_category_title,report_categories.id as report_category_id,report_categories.title as report_category_title,reports.*');
		$this -> pdo -> where(array('reports.order_id' => $order_id,'reports.enable' => TRUE,'gene_categories.enable'=>TRUE,'report_categories.enable'=>TRUE));
		$this -> pdo -> join('gene_categories','reports.gene_category_id = gene_categories.id');
